...
const TUNING_LOG = [
    ...
    {
        cluster: 'R8 stages 1, 5, 7 + r8_boss (BUGS #11 freewin fallout)', file: 'js/missions.js',
        was: 'r8_s1 count 6,6,7,7 · r8_s5 count 6,7,7,7 · r8_s7 count 6,7,7,7 (budgets unchanged) · void_sovereign atkScaling 0.75',
        now: 'r8_s1 count 8,8,9,9 · r8_s5 count 8,9,9,9 · r8_s7 count 8,9,9,9 (budgets unchanged -- see why) · void_sovereign atkScaling 0.95',
        why: 'Prompt 74 (BUGS #11) implemented the 12 previously-no-op unit abilities (ashen_watcher, abyssal_guardian, grove_warden, tempest_weaver, voltfang_stalker, iron_duelist + evolved forms). These are real T4 units buildReferenceTeam() already picks into R8\'s reference team (e.g. abyssal_guardian, ashen_watcher) -- fixing their kits made that team meaningfully stronger, flipping r8_s1/r8_s5/r8_s7/r8_boss from Prompt 66\'s tuned "near-death win" margin into 0-loss freewins (r4_s1\'s pre-existing freewin flipped the other way and self-resolved -- no action needed there). `budget` was tried first and found to be a no-op lever at these stages: generateMissionWave()\'s enemy-picking loop is bounded by `count`, not by spending the full budget, and budget here already covers count*maxCost with room to spare -- confirmed empirically (2.1x budget on the pre-fix values produced byte-identical sim output to 1x). `count` (+2/wave on the 3 flagged stages) was the effective, smallest-viable lever; r8_boss got a data-only atkScaling nudge (mechanics untouched, same lever Prompt 66 used on this boss). Re-verified via tests/balance-sim.js: 0 walls introduced (still exactly the pre-existing r1_s4/r1_s8/r3_s8), 0 freewins remaining anywhere.'
    },
    {
        cluster: 'R8 stage 2 + r8_boss (BUGS #12 passive-fallout, round two)', file: 'js/missions.js',
        was: 'r8_s2 count 6,7,7,7 (budgets unchanged) · void_sovereign atkScaling 0.95',
        now: 'r8_s2 count 8,9,9,9 (budgets unchanged -- see why) · void_sovereign atkScaling 1.15',
        why: 'Prompt 75 (BUGS #12) implemented the same 6 units\' innate PASSIVE_DATA (Ember Shroud, Pressure Depths, Deep Roots, Lingering Gales, Overcharge Frenzy, Challenge Protocol + evolved) on top of Prompt 74\'s ability fix -- and, while wiring the on_heal/on_crit/on_ability_cast dispatch, found and fixed a pre-existing blocking bug where unit.isEvolved was never set anywhere in the live combat pipeline, so getPassiveData() silently returned null for EVERY evolved unit in the game (not just this prompt\'s 6) -- see js/combat-core.js\'s `su.isEvolved = ...` comment. Both changes made buildReferenceTeam()\'s R8 picks stronger a second time, re-flipping r8_s2 and r8_boss (already once-tuned by Prompt 74) back into 0-loss freewins; r8_s1/s5/s7 (Prompt 74\'s other 3 flagged stages) held at their tuned margins and needed no further change. Same levers/reasoning as Prompt 74\'s entry above (`count` +2/wave on r8_s2, a further void_sovereign atkScaling nudge) -- re-verified via tests/balance-sim.js: 0 walls introduced (still exactly r1_s4/r1_s8/r3_s8), 0 freewins remaining anywhere, r8_s2 at 7.9/8 survivors and r8_boss at 7.0/8 survivors (a real, non-zero casualty count at 100% clear, matching the target profile every other R8 stage/boss already has).'
    }
];
